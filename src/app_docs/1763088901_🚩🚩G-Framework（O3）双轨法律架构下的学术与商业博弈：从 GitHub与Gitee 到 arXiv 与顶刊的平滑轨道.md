...
2. 以“GitHub/Gitee（NC-ND 渊源 + 证据指纹） + Zenodo DOI（`S_{base}` 确权） → arXiv（引用 DOI 做广播塔） → 顶刊/商业授权” 为主线的发表路线，构成一条**通向高门槛的平滑轨道**：每一阶段都单调提高作者的优先权收益 ($\Pi^{\mathrm{prio}}$)、话语权收益 ($\Pi^{\mathrm{vis}}$) 与法律地位 ($\Pi^{\mathrm{legal}}$)，同时对潜在商业侵权者施加强烈的结构性约束。
...
*   ($S_3$)：在 Zenodo 归档生成 DOI (`S_{base}`)，挂载（专著 PDF + GitHub/Gitee Snapshot）并同步上传 arXiv，在元数据中引用该 DOI，形成 ($B_{\mathrm{arXiv}}$)；
