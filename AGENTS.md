...
**编码与换行（最高优先级）**
- 文档一律使用 `UTF-8` 编码。
- 文档行尾一律使用 `CRLF`（Windows 样式）。
- 本条为最高优先级规范，覆盖此前“尽量保留原有换行风格（CRLF/LF）”的建议；所有自动化脚本/批处理在生成或改写文档时也须输出为 `UTF-8 + CRLF`。
- 与 `src/kernel_reference` 子树相关的“避免格式抖动”等既有约束保持不变；仅在换行风格上从“尽量保持原有”统一为 `CRLF`，不得改动手动维护的声明区或其他固定结构。
