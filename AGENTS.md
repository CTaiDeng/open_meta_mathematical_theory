...
**编码与换行（最高优先级）**
- 文档一律使用 `UTF-8（无BOM）` 编码。
- 文档行尾一律使用 `LF`（Unix 样式）。
- 本条为最高优先级规范，覆盖此前“尽量保留原有换行风格（CRLF/LF）”的建议；所有自动化脚本/批处理在生成或改写文档时也须输出为 `UTF-8（无BOM） + LF`。
- 与 `src/kernel_reference` 子树相关的“避免格式抖动”等既有约束保持不变；仅在换行风格上从“尽量保持原有”统一为 `LF`，不得改动手动维护的声明区或其他固定结构。

**文档与编码**
- 文档默认使用 UTF-8（无BOM）编码；行尾统一使用 LF。
...
- 自动化脚本输出需避免不必要的格式“抖动”，统一使用 UTF-8（无BOM）编码，换行统一使用 LF。
